...
export function MissionSection() {
  ...
  return (
    <section ref={sectionRef} className="relative py-24 sm:py-32 bg-background overflow-hidden">
      <MouseGlow color="hsl(28, 85%, 55%)" size={600} opacity={0.06} />
          ...
          <ScrollReveal direction="right">
            <div className="relative group">
              ...
              >
                <img
                  src={missionImg}
                  alt="Virtual try-on technology on smartphone"
                  className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                />
                <div className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/10 transition-all duration-500" />
              </motion.div>
              {/* Accent border frame */}
              <div className="absolute -bottom-4 -right-4 w-full h-full rounded-2xl -z-10"
                style={{ border: "1px solid hsl(28, 85%, 55%, 0.2)" }} />
              {/* Corner accents in teal */}
              <motion.div
                className="absolute -top-2 -left-2 w-8 h-8 border-t-2 border-l-2 rounded-tl-lg"
                style={{ borderColor: "hsl(175, 65%, 45%, 0.5)" }}
              ...
              <motion.div
                ...
                style={{ borderColor: "hsl(28, 85%, 55%, 0.5)" }}
                initial={{ opacity: 0, scale: 0 }}
                whileInView={{ opacity: 1, scale: 1 }}
                viewport={{ once: true }}
